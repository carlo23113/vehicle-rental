  ...
  <div class="mt-[2.5rem]">
    <CommonFormSection title="Vehicle Photos" icon="mdi-camera">
      <v-row>
        <v-col cols="12">
          <div
            class="border-2 border-dashed rounded-xl transition-all duration-300 border-[rgba(var(--v-border-color),0.2)] hover:border-primary/30 hover:bg-primary/[0.03]"
          >
            <input
              ref="fileInputRef"
              type="file"
            ...
            <div
              v-if="allPhotosComputed.length === 0"
              class="p-12 text-center cursor-pointer group"
              @click="fileInputRef?.click()"
            >
              <v-icon
                icon="mdi-cloud-upload"
                size="64"
                class="opacity-40 transition-all duration-300 group-hover:opacity-60 group-hover:scale-110"
              />
              ...
              <v-btn color="primary" variant="tonal" class="mt-4" prepend-icon="mdi-upload">
                Select Photos
              </v-btn>
            </div>
            ...
            <div v-else class="grid grid-cols-[repeat(auto-fill,minmax(180px,1fr))] gap-4 p-4">
              ...
              <div
                v-if="allPhotosComputed.length < maxPhotos"
          ...
          <div class="flex items-center justify-between mt-3">
            ...
            <p
              ...
              :class="
                allPhotosComputed.length === maxPhotos ? 'text-warning' : 'text-medium-emphasis'
              "
            >
...
const handleFileSelect = (event: Event) => {
  ...
  if (remainingSlots === 0) {
    emit(
      'error',
      `Maximum of ${props.maxPhotos} photos allowed. Please remove some photos before adding more.`
    )
  ...
  Array.from(files).forEach(file => {
    ...
    if (!file.type.startsWith('image/')) {
      emit(
        'error',
        `"${file.name}" is not a valid image file. Only JPG, PNG, and WEBP are supported.`
      )
      return
    }
  ...
  if (skippedFiles.length > 0) {
    emit(
      'warning',
      `${addedCount} photo(s) added. ${skippedFiles.length} file(s) skipped (maximum ${props.maxPhotos} photos allowed).`
    )
  }
...
const allPhotosComputed = computed<Photo[]>(() => {
  const existing: ExistingPhoto[] = displayedExistingPhotos.value.map(url => ({
    url,
