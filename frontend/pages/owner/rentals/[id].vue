<template>
  <CommonPageContainer>
    ...
    <CommonPageDetailPageHeader
      ...
      @back="$router.push('/owner/rentals')"
      @action="handleAction"
    />
...
// Check if rental has a draft invoice (simulate - in real app, fetch from API)
const hasDraftInvoice = ref(false)
const draftInvoiceId = ref<string | null>(null)

// Simulate auto-generation of draft invoice when rental is viewed
// In real app, this would be done when rental status changes to 'active' or 'reserved'
onMounted(() => {
  // Simulate checking if invoice exists for this rental
  // In production, this would be an API call
  const existingInvoice = localStorage.getItem(`invoice-draft-rental-${rental.value.id}`)
  if (existingInvoice) {
    hasDraftInvoice.value = true
    draftInvoiceId.value = JSON.parse(existingInvoice).id
  } else if (rental.value.status === 'active' || rental.value.status === 'reserved') {
    // Auto-generate draft invoice
    autoGenerateDraftInvoice()
  }
})

const autoGenerateDraftInvoice = () => {
  // Create draft invoice automatically
  const draftInvoice = {
    id: `draft-${rental.value.id}`,
    rentalId: rental.value.id,
    status: 'draft',
    createdAt: new Date().toISOString(),
  }

  localStorage.setItem(`invoice-draft-rental-${rental.value.id}`, JSON.stringify(draftInvoice))
  hasDraftInvoice.value = true
  draftInvoiceId.value = draftInvoice.id

  // Show notification
  snackbar.value = {
    show: true,
    message: 'Draft invoice auto-generated. Review and send when ready.',
    color: 'success',
...
const getInvoiceButtonState = () => {
  ...
  const hasInv = hasDraftInvoice.value

  // Cancelled or Reserved rentals - don't show invoice button
  // Reserved: invoice auto-generated, no action needed
  // Cancelled: no invoice needed
  if (status === 'cancelled' || status === 'reserved') {
    return null
  }

  // Active or Completed WITHOUT invoice = ERROR state (should never happen)
  if ((status === 'active' || status === 'completed') && !hasInv) {
    return {
      label: 'Missing Invoice - Create Now',
...
const handleAction = (key: string) => {
  if (key === 'edit') {
    router.push(`/owner/rentals/edit/${rental.value.id}`)
  } else if (key === 'print') {
    showContractDialog.value = true
  } else if (key === 'invoice') {
    // Always navigate to invoice page with rentalId
    // Will view existing or create new based on hasDraftInvoice
    router.push(`/owner/invoices/new?rentalId=${rental.value.id}${hasDraftInvoice.value ? '&draft=true' : ''}`)
  }
}
